          ...
          <T.Head className="text-nowrap">
            {columns?.map((col) => (
              <T.HeadCell
                key={col?.id}
                scope="col"
                className={`${col?.order || col?.id === 'checkbox' ? '' : 'pointer-events-none'} p-4 bg-sinabe-gray-dark text-white ${col?.id !== 'checkbox' ? 'cursor-pointer hover:bg-purple-500 hover:text-white transition-colors ease-in-out duration-100' : ''} ${col?.id === sortedBy && 'bg-purple-500'} ${col?.classes}`}
