...
const ImageViewer = ({ images = [], onRemove, imagesClassNames }) => {
  ...
  useEffect(() => {
    if (selectedImage !== null && imageRefs.current[selectedImage]) {
      imageRefs.current[selectedImage].click();
    } else {
        ...
        images.map((image, index) => (
          <div
            ...
            className={classNames(
              'relative shadow-md w-24 h-24 xl:h-28 xl:w-28 2xl:h-32 2xl:w-32',
              classNames,
            )}
          >
            {onRemove && (
            ...
            {image instanceof File ? (
              <LazyLoadImage
                src={FormattedUrlImage(image)}
                alt={`Image ${index + 1}`}
                className="w-full h-full object-cover cursor-pointer shadow-sm rounded-md"
                onClick={() => handleSelectImage(index)}
              />
            ) : (
              <LazyLoadImage
                src={FormattedUrlImage(image.thumbnail)}
                alt={`Image ${index + 1}`}
                className="w-full h-full object-cover cursor-pointer shadow-sm rounded-md"
                onClick={() => handleSelectImage(index)}
              />
